...
export async function getMensalistasComPendenciaAction(arenaId: string) {
  try {
    ...
    const { data: planos, error } = await supabase
      ...
      .select(
        'id, athlete_id, athlete_name, valor_mensal, sessoes_por_mes, dia_semana, horario_inicio, horario_fim, data_inicio, atleta:athlete_id(id, nome_perfil, telefone), court:court_id(id, name), sports:sport_id(id, name)'
      )
      .eq('arena_id', arenaId)
      .eq('status', 'ativo')
    ...
    const withPending = await Promise.all(
      (planos || []).map(async (plano: any) => {
        ...
        return {
          ...plano,
          proximo_mes_reservado: reservado?.[0]?.start_time || null,
        };
      })
    );

    const pendentes = withPending.filter(
      (p) => p.proximo_mes_reservado !== null
    );
    return { success: true, data: pendentes };
  } catch (err) {
    const message =
      err instanceof Error ? err.message : 'Erro ao buscar pendências';
    return { success: false, error: message, data: [] };
  }
}
